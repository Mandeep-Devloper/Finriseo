  ...
  try {
    ...
    if (isMsg91Configured()) {
      ...
      if (!sent.ok) {
        ...
        console.error(`[OTP SEND] MSG91 failed for ${mobile}: ${sent.error}`);
        return NextResponse.json(
      ...
      // MSG91 not configured — log to console; expose OTP only in dev.
      console.log(`[OTP SEND] Mobile: ${mobile} | OTP: ${otp} (MSG91 not configured)`);
